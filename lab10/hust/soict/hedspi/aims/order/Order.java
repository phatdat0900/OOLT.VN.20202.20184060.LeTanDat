...
public class Order {
	...
	public void addMedia(Media obj1, Media obj2) {
		...
		if (this.itemsOrdered.size() + 1 <= MAX_NUMBERS_ORDERED) {
			...
			//assert this.itemsOrdered.size() <= MAX_NUMBERS_ORDERED : "You can order 10 medias/order ";
			throw new ArithmeticException("You can order 10 medias/order ");
		}
	...
	public Media getALuckyItem(float x) {
		int [] index = new int [itemsOrdered.size()];
		int j =0;
		if(itemsOrdered.size() > 5 && this.totalCost() >50) {
			for(int i = 0 ; i< itemsOrdered.size();i++) {
				if(itemsOrdered.get(i).getCost() > x/10) {
					index[j++]= i;
				}
				if(j!= 0) {
					 int rnd = new Random().nextInt(index.length);
					 return itemsOrdered.get(rnd);
				}else {
					JOptionPane.showMessageDialog(null, "None of the items you have ordered satisfy the condition ", "Error", JOptionPane.ERROR_MESSAGE);
			...
			JOptionPane.showMessageDialog(null, "You can get a lucky item", "Error", JOptionPane.ERROR_MESSAGE);
